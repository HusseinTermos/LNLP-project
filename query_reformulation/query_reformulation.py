...
from query_reformulation.query_reform_prompt import PROMPT_TEMPLATE
...
class Query_Reformulater:
    ...
    def _clean(text: str) -> str:
        text = text.strip()
        if text.upper().startswith("SIMPLIFIED CLAIM:"):
            text = text[len("SIMPLIFIED CLAIM:"):].strip()
        return text.split("\n")[0].strip()
    ...
    def _inference_local(self, prompts):
        ...
        outputs.extend([self._clean(text) for text in batch_outputs])
        ...
        if self.cache is not None:
            for i, claim in enumerate(claims):
                ...
                if key in self.cache:
                    results[i] = self._clean(self.cache[key])
                else:
